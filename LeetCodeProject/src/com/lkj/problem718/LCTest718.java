...
/** 动态规划，参考自己《两个数组/字符串的最长公共子序列》的套路题总结
 dp[i][j]的定义：以A[i]结尾的子数组集合 与 以B[j]结尾的子数组集合中 的最长的公共子数组的长度（注意，这些公共数组必然也是分别以 A[i]，B[j]结尾！）。
 状态方程为：
 1）如果 A[i] == B[j]，dp[i][j] = dp[i - 1][j - 1] + 1
 2）否则，dp[i][j] = 0 （A的子数组集合结尾 与 B的子数组集合的结尾 都不相同，那么A子数组集合中的所有子数组必然都与B子数组集合的所有子数组不同，
    ，因此，此时A[i]、B[j]结尾的子数组集合 中没有公共子数组，此时公共子数组长度为0）
