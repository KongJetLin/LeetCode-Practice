...
1）这个题目可以将x变为字符串，然后拆开为字符来解答，但是这样涉及多个字符的操作，复杂度较高；
 ...
 2）使用数学方法，类似于入栈出栈操作，将x的最后一个数字取出，同时将x/10，用于取出下一个数字。
...
但是，这里必须考虑到溢出问题，int的最大值：MAX_VALUE 2147483647，最小值：MIN_VALUE -2147483648
 即使一个数字不溢出，它翻转后可能溢出。
 此处的关键是必须在 rev 加 pop 之前判断rev加pop后是否越界，否则 rev 加上 pop 之后有可能溢出！！！！（先判断后加！）
...
public class LCTest7_2
{
    public int reverse(int x)
    {
        ...
        while(x!=0)
        {
            ...
            /* 必须在 rev 加 pop 之前判断rev加pop后是否越界，否则 rev 加上 pop 之后有可能溢出！*/
